...
class Line extends Component {
  ...
  handleMouseClick(evt) {
    ...
    return (
      onClick &&
      onClick(line, [projection(line.coordinates.start), projection(line.coordinates.end)], evt)
    );
  }
  handleFocus(evt) {
    evt.persist();
  ...
  render() {
    ...
    const path = buildPath ? buildPath(start, end, line) : `M ${start.join(' ')} L ${end.join(' ')}`;
